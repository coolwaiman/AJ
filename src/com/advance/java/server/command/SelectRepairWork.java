...
public class SelectRepairWork implements Command  {
    ...
    public void execute() throws IOException {
        session.out.println("Please enter the selected repair work id");
        ...
        try{
            ...
            if(work!=null)
                ...
                session.out.println("No record found.Please enter the correct repair work id");
    ...
    private void printWork(Productrepairwork work){
        ...
        if(work.getSolvedDate()!=null||work.getSolvedDate().getTime()!=0)
            ...
            solvedDate="N/A";
        session.out.printf(TABLE_ROW_FORMAT,"Repair Work ID",work.getRepairWorkId());
        session.out.printf(TABLE_ROW_FORMAT,"Order Id",work.getOrder().getOrderId());
        session.out.printf(TABLE_ROW_FORMAT,"Product SN",work.getStoreProduct().getProductSn());
        session.out.printf(TABLE_ROW_FORMAT,"Product ID",work.getStoreProduct().getProduct().getProductId());
        session.out.printf(TABLE_ROW_FORMAT,"Product Name",work.getStoreProduct().getProduct().getProductName());
        session.out.printf(TABLE_ROW_FORMAT,"Assigned Technician ID",work.getAssignedTechnician().getStaffId());
        session.out.printf(TABLE_ROW_FORMAT,"Assigned Technician Name",work.getAssignedTechnician().getStaffName());
        session.out.printf(TABLE_ROW_FORMAT,"Request Created Date",createDate);
        session.out.printf(TABLE_ROW_FORMAT,"Solved Date",solvedDate);
    }
}
